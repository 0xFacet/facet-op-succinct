...
use crate::{
    rollup_config::RollupProposerConfig,
    rollup_contract::Rollup::{RollupInstance, ProposalSubmitted, ProposalStatus},
    rollup_prometheus::ProposerGauge,
    ...
    P: Provider + Clone + Send + Sync,
{
    ...
    async fn get_proposals_length(&self) -> Result<U256> {
        Ok(self.rollup.getProposalsLength().call().await?)
    }
    ...
    pub async fn create_proposal(
        ...
        Ok(*self.rollup.address())
    }

    /// Find the latest *valid* proposal whose root matches the true output root.
    /// Mirrors the structure and logging style of the upstream implementation used for games.
    async fn get_latest_valid_proposal(&self) -> Result<Option<(U256, U256)>> {
        // Get latest proposal id; return None if no proposals exist (only genesis).
        let mut proposal_id = match self.get_proposals_length().await? {
            len if len > U256::ZERO => len - U256::from(1),
            _ => {
        ...
        // Loop through proposals in reverse order (latest to earliest) to find the most recent valid proposal.
        loop {
            // Get the proposal for the current index.
            let proposal = self.rollup.getProposal(proposal_id).call().await?;

            // Get the L2 block number the proposal is advancing.
            ...
            // If the output root matches the proposal claim, we've found the latest valid proposal.
            if output_root == proposal.rootClaim {
                break;
            }

            // If the output root doesn't match the proposal claim, log and look at earlier proposals.
            tracing::info!(
            ...
            // If we've reached index 0 (the earliest proposal) and still haven't found a valid proposal.
            if proposal_id == U256::ZERO {
                tracing::info!("No valid proposals found after checking all proposals");
                return Ok(None);
            }

            // Decrement the proposal id to check the previous proposal.
    ...
    pub async fn handle_proposal_creation(&self) -> Result<Option<Address>> {
        ...
        let (reference_block, reference_proposal_id) = match self.get_latest_valid_proposal().await? {
        ...
        if let Some(finalized_block) = finalized_l2_head_block_number {
            ...
            if U256::from(finalized_block) > U256::from(next_l2_block_number) {
                ...
                let proposal_address = self.create_proposal(U256::from(next_l2_block_number)).await?;

                Ok(Some(proposal_address))
            } else {
                tracing::info!(
    ...
    pub async fn handle_proposal_resolution(&self) -> Result<()> {
        ...
        let start_id = proposals_length.saturating_sub(U256::from(self.config.max_games_to_check_for_resolution));
    ...
    pub async fn handle_proposal_defense(&self) -> Result<()> {
        ...
        let proposals_length = self.get_proposals_length().await?;
        let anchor_id = U256::from(self.rollup.anchorProposalId().call().await?);
        let start_id = proposals_length.saturating_sub(U256::from(self.config.max_games_to_check_for_defense));
        let start_id = start_id.max(anchor_id);
        
        let mut defended_count = 0;
        
        for i in 0..self.config.max_games_to_check_for_defense {
            let proposal_id = start_id + U256::from(i);
            if proposal_id >= proposals_length {
                break;
            }
